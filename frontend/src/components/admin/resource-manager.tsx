...
import { useAdminList, useResourceMutations } from "@/lib/queries";
...
export function ResourceManager({ config }: { config: ResourceConfig }) {
  const { data, isLoading } = useAdminList<Item>(config.resource as ResourceKey);
  const { create, update, remove: removeMut } = useResourceMutations(
    config.resource as ResourceKey,
  );
  const items = data ?? [];
